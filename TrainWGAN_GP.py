...
class TrainWGAN_GP(object):
    ...
    def train(self):
        ...
        if self.resume:
            ...
            #################correct for huge GP in the beginning####################
            self.iter_result['lossD'][:25] = self.iter_result['lossD'][25:50]
            for i in range(start_epoch-1):
                self.epoch_result['lossD'][i] = np.mean(
                    np.array(self.iter_result['lossD'][:ITER_PER_EPOCH*(i+1)]))
